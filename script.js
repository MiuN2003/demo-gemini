...
function wrapWordsWithSpan(wordInfo) {
    ...
    let wrappedWords = words.map((word, index) => {
        ...
        let id = Object.keys(wordInfo).find(id => wordInfo[id].Word.toLowerCase() === cleanWord.toLowerCase());
        return id ? `<span id="word-${id}" class="word">${word}</span>` : word;
    });
...
function addDoubleClickEvent(wordInfo) {
    ...
    document.querySelectorAll('.word').forEach(span => {
        span.addEventListener('dblclick', (event) => {
            let selectedID = event.target.id.replace('word-', '');
